function [cepstrograma,fo,tfo] = cepstrogram_beta(senal,T,ancho_ventana,corrimiento)
...
cepstrograma=zeros(length(cepstrum(tcep>=2e-3 & tcep<=20e-3)),floor((length(senal)-length(w))/paso));
for i=paso:paso:length(senal)-length(w)
    
    senal_ventana=w.*senal(i:i+n_ventana-1);
    cepstrum=rceps(senal_ventana);
    crng= cepstrum(tcep>=2e-3 & tcep<=20e-3);
    crng_graph= cepstrum(tcep>=2e-3 & tcep<=20e-3);
